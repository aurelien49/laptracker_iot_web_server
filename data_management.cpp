...
std::vector<CustomDataStruct>& DataManagement::getdataList() {
  return dataList;
}
...
void DataManagement::recordingData(unsigned long currentTime) {
  if (currentTime - lastToggleTime >= RECORDING_TIME) {
    ...
    if (dataList.size() < MAX_RECORDS) {
      CustomDataStruct data = { dateAndTimeM.year(), dateAndTimeM.month(), dateAndTimeM.day(), dateAndTimeM.hour(), dateAndTimeM.minute(), dateAndTimeM.second(), temperature, humidity, roll, pitch, yaw };
...
void DataManagement::setdataList(const std::vector<CustomDataStruct>& values) {
  dataList = values;
}

void DataManagement::addTemperatureHumidity(CustomDataStruct values) {
